...
def extend_multi_concepts(df, effective_keys):
    if "questions" not in effective_keys or "concepts" not in effective_keys:
        ...
        return df, effective_keys
    # Only split comma-sep sequence columns; exclude uid and fold (int per row)
    extend_keys = set(df.columns) - {"uid", "fold"}
    ...
    for _, row in df.iterrows():
        ...
        for key in extend_keys:
            val = row[key]
            dextend_infos[key] = val.split(",") if isinstance(val, str) else [str(val)]
        dextend_res = dict()
...
def _to_comma_sep_ids(val):
    """Convert a value to a list of id strings (comma-sep or single int/float)."""
...
def id_mapping(df):
    ...
    for i, row in df.iterrows():
        for key in id_keys:
            ...
            dres.setdefault(key, [])
            curids = []
            for id in _to_comma_sep_ids(row[key]):
...
def main_from_dataframe(total_df, dname, dataset_name, configf, effective_keys, min_seq_len=3, maxlen=200, kfold=5):
    """Run pykt preprocessing from a pre-built dataframe with fold column (e.g. LIVE_KT2 train/test split).

    Args:
        total_df (pd.DataFrame): one row per user, columns: uid, questions (comma-sep), responses (comma-sep),
            fold (int: 0..k-1 for train, -1 for test), and optionally concepts (comma-sep).
        dname (str): data folder path (will be created if needed)
        dataset_name (str): dataset name for config
        configf (str): path to data_config.json
        effective_keys (set): e.g. {"uid", "questions", "responses", "concepts"} (fold is added internally)
        min_seq_len (int): min sequence length
        maxlen (int): max sequence length
        kfold (int): number of folds for train (folds 0..k-1)
    """
    os.makedirs(dname, exist_ok=True)
    stares = []

    if "concepts" in total_df.columns and "concepts" not in effective_keys:
        effective_keys = set(effective_keys) | {"concepts"}
    if "questions" in total_df.columns and "questions" not in effective_keys:
        effective_keys = set(effective_keys) | {"questions"}

    total_df, effective_keys = extend_multi_concepts(total_df.copy(), effective_keys)
    total_df, dkeyid2idx = id_mapping(total_df)
    if "concepts" in effective_keys:
        dkeyid2idx["max_concepts"] = get_max_concepts(total_df)
    else:
        dkeyid2idx["max_concepts"] = -1
    ...
    splitdf = total_df[total_df["fold"] >= 0].copy()
    test_df = total_df[total_df["fold"] == -1].copy()
    test_df["fold"] = [-1] * len(test_df)
    ...
    flag2, test_question_window_seqs = generate_question_sequences(test_df, effective_keys, True, min_seq_len, maxlen)
    flag = flag and flag2
